...
public class MonsterSensor : MonoBehaviour
{
    ...
    void Start()
    {
        MonsterConfig = GetComponent<MonsterController>().Config;
    ...
    public void ForceDetection(GameObject attacker)
    {
        ...
        {   // 방어 코드
            if (attacker == null)
                Debug.LogError($"[MonsterSensor] ForceDetection의 attacker인수가 null입니다.");
            return;
        }

        // attacker의 거리가 감지할수도 없이 너무 멀면 포기.
        float distance = (attacker.transform.position - transform.position).magnitude;
        if (distance >= MonsterConfig.fovRange * rangeMultiplier && attacker.TryGetComponent<IPlayerControllable>(out _))
        {
            SetTarget(attacker);
            isOnSight = true;
    ...
    bool IsTargetOnSensor(GameObject target)
    {
        if (target == null)
        {   // 방어코드
            Debug.LogError($"[MonsterSensor] Target Check의 인수인 target이 null입니다.");
            return false;
        }

        Vector3 sightPos = transform.position + Vector3.up * MonsterConfig.fovHeight;
        Vector3 targetPos = target.transform.position;
        Vector3 direction = (targetPos - sightPos).normalized;
        float distance = Vector3.Distance(transform.position, targetPos);

        isOnSight = CheckFov(distance, sightPos, direction);
        isOnHeard = CheckSound(distance, target);
    ...
    {   // 시야 감지 로직
        isOnSight = false;
        if (distance <= MonsterConfig.fovRange)
        {   // 시야각 감지
            ...
            if (Vector3.Angle(forwardSensor, targetDirection) < MonsterConfig.fovAngle / 2)
            {
    ...
    bool CheckSound(float distance, GameObject target)
    {   // 사운드 감지 로직
        isOnHeard = false;
        if (distance <= MonsterConfig.soundRange)
        {   // 소리 감지
    ...
    {
        // 방어코드
        if (!debug || !MonsterConfig)
            return;

        Gizmos.color = Color.yellow;
        Gizmos.DrawWireSphere(transform.position, MonsterConfig.soundRange);

        Gizmos.color = Color.red;
        Gizmos.DrawWireSphere(transform.position, MonsterConfig.fovRange);

        Vector3 fovLine1 = Quaternion.AngleAxis(MonsterConfig.fovAngle / 2, transform.up) * transform.forward * MonsterConfig.fovRange;
        Vector3 fovLine2 = Quaternion.AngleAxis(-MonsterConfig.fovAngle / 2, transform.up) * transform.forward * MonsterConfig.fovRange;
        ...
        if (target)
        {
            ...
            Gizmos.DrawLine(transform.position + Vector3.up * MonsterConfig.fovHeight, target.transform.position);
        }
    }
    #endregion
}
